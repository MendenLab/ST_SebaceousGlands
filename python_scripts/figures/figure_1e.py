...
def main(adata, save_folder):
    ...
    sns.boxplot(data=df_melt, x='gene_name', y='value', hue='condition', fliersize=0, ax=ax, palette=palette)
    sns.stripplot(data=df_melt, x='gene_name', y='value', hue='condition', jitter=True,
                  dodge=True, marker='.', color='black', ax=ax, legend=False)
    # Add annotations
    ...
    plt.savefig(os.path.join(save_folder, 'Boxplots_normed_AD_Pso.pdf'), bbox_inches='tight')
    plt.close()
