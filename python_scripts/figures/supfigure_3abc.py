...
def main(input_folder, save_folder):
    ...
    df_patter_0 = pd.read_excel(os.path.join(input_folder, filename), sheet_name='0')
    df_patter_0['log_p.adjust'] = -np.log10(df_patter_0['p.adjust'])
    df_patter_6 = pd.read_excel(os.path.join(input_folder, filename), sheet_name='6')
    df_patter_6['log_p.adjust'] = -np.log10(df_patter_6['p.adjust'])
    df_patter_8 = pd.read_excel(os.path.join(input_folder, filename), sheet_name='8')
    df_patter_8['log_p.adjust'] = -np.log10(df_patter_8['p.adjust'])

    # Scatterplot of padj val of pathways of Pattern 0 vs padj val of pathways od Pattern 8
    # Note: Description not unique but ID is
    df_08 = df_patter_0[['ID', 'Description', 'log_p.adjust']].merge(
        df_patter_8[['ID', 'Description', 'log_p.adjust']], on=['ID'], how='outer')
    df_08['log_p.adjust_x'] = df_08['log_p.adjust_x'].fillna(0)
    df_08['log_p.adjust_y'] = df_08['log_p.adjust_y'].fillna(0)

    plot_scatter(df=df_08, save_folder=save_folder, pattern_name_1='0', pattern_name_2='8')

    df_06 = df_patter_0[['ID', 'Description', 'log_p.adjust']].merge(
        df_patter_6[['ID', 'Description', 'log_p.adjust']], on=['ID'], how='outer')
    df_06['log_p.adjust_x'] = df_06['log_p.adjust_x'].fillna(0)
    df_06['log_p.adjust_y'] = df_06['log_p.adjust_y'].fillna(0)
    plot_scatter(df=df_06, save_folder=save_folder, pattern_name_1='0', pattern_name_2='6')

    df_68 = df_patter_6[['ID', 'Description', 'log_p.adjust']].merge(
        df_patter_8[['ID', 'Description', 'log_p.adjust']], on=['ID'], how='outer')
    df_68['log_p.adjust_x'] = df_68['log_p.adjust_x'].fillna(0)
    df_68['log_p.adjust_y'] = df_68['log_p.adjust_y'].fillna(0)
    plot_scatter(df=df_68, save_folder=save_folder, pattern_name_1='6', pattern_name_2='8')
...
if __name__ == '__main__':
    ...
    input_path = os.path.join(input_dir, 'spatialDE/2023-04-12_paper_figures/Pathway_enrichment_analysis',
                              'Figure2def/patient/ORA_REACTOME/Patterns/2023-07-17/11-V19T12-012-V2')
